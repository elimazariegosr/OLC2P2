...
import {Arreglo, Push, Pop, SD_Arreglo, GD_Arreglo} from './Instrucciones/Arreglo';
import {Arreglo_3d, Get_Arreglo_3d, Set_Arreglo_3d, Asig_Arreglo_3d} from './Instrucciones/Arreglo_3d';
...
export class ContenidoComponent implements OnInit {
  ...
  /*Traduccion 3 direcciones*/
  traducir_3d(entrada: string){
    this.tabla = new Tabla(null); // Inicializar la tabla de simbolos
    document.getElementById("tablas").innerHTML ="";// limpiando el div que contiene tablas de erores y ts  
    ...
    console.log(this.arbol);
    if(this.desanidacion.hay_anidada(this.arbol)){// preguntar si existe alguna funcion anidada...
      this.desanidacion.desanidar(this.arbol);// obtener la desanidacion y traduccion
    }
    this.arbol.tabla_global = this.tabla;
    let cont  = "";
    this.arbol.instrucciones.forEach(element => {//primeraa pasada para guardar las funciones declaradas
      if(element instanceof Arreglo_3d){
        element.traducir(this.tabla, this.arbol);
      }
    });
    
    cont  = this.arbol.contenido;
    this,this.arbol.contenido = "";
    ...
    this.arbol.cabecera += this.arbol.funciones_extra;
    document.getElementById('txt_traduccion').innerHTML = this.arbol.cabecera + "\n";  // mostrar la traduccion en el text area
    document.getElementById('txt_traduccion').innerHTML += this.arbol.contenido;  // mostrar la traduccion en el text area
