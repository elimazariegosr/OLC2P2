...
class Llamada_funcion extends Nodo{
    ...
    traducir(tabla: Tabla, arbol: Arbol){
        ...
        if(this.funcion == null){
            let inicio = arbol.contenido.length;
            let fin = 0;
            let cont_aux = arbol.contenido;
            let declarada = false;
            for(let i = 0; i < arbol.instrucciones.length; i++){
                let isnt = arbol.instrucciones[i];
                if(isnt instanceof Funcion){
                    if(isnt.id == this.id){
                        declarada = true;
            ...
            if(declarada){
                fin  = arbol.contenido.length;
                for(let i = inicio; i < fin; i++){
                    arbol.funciones_extra += arbol.contenido[i];
                }
                arbol.contenido = cont_aux;    
                this.buscar_funcion(tabla);
            }
        }
        if(this.funcion instanceof Funcion){
